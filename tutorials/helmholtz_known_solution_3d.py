"""Short 3D Helmholtz example with a known solution."""
...
kh = 4.0
source = torch.tensor([11.0, 11.0, 11.0])


def exact_solution(xx):
    return uu_dir_func_greens(3, xx, kh, center=source)
...
def main():
    ...
    operator = pdo.PDO_3d(
        pdo.ones, pdo.ones, pdo.ones, c=pdo.const(-kh**2)
    )

    solver = Domain_Driver(BoxGeometry(box), operator, kh, a, p=8, d=3)
    ...
    print(f"3D relative error: {relative_error:.3e}")
