...
from hps.domain_driver     import Domain_Driver #from hps.hps_multidomain   import HPSMultidomain
...
def get_discretization_relerr(a,p,kh,ndim,elongated_x=False,elongated_y=False,sparse_assembly='reduced_gpu',solver_type='MUMPS'):
	...
	geom = box.T #BoxGeometry(box)

	solver    = Domain_Driver(geom,pdo,0,a,p,d=ndim)
	solver.build(sparse_assembly, solver_type,verbose=True)
